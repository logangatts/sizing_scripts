...
function getlambdas {
  aws --profile $profile lambda list-functions  --region $r --output json --no-paginate | jq '.[] | length'
}
...
function calculateInventory {
  ...
  for r in $(getRegions); do
    ...
    lambdainv=$(getlambdas $r $profile)
    LAMBDA=$(($LAMBDA + $lambdainv))
done

#s3 is global so only needs to be run once for accurate counts (not per region)
S3= $(gets3)

TOTAL=$(($EC2_INSTANCES + $RDS_INSTANCES + $REDSHIFT_CLUSTERS + $ELB_V1 + $ELB_V2 + $NAT_GATEWAYS + $LAMBDA + $S3))
}
